 ...
 *
 * <p>Players not in this list will be assigned anonymous numeric identifiers instead of having
 * their usernames recorded in match data files.
 *
...
public class PermittedPlayers {
   ...
   *
   * <p>If the configuration file doesn't exist, a default template is created from the plugin's
   * resources. Invalid UUIDs in the config file are logged and skipped.
   *
   * <p>This method clears the existing list before loading, so it can be called to reload changes
  ...
  public void load() {
    ...
    for (String uuidStr : config.getStringList("permitted")) {
      try {
        UUID uuid = UUID.fromString(uuidStr);
        int playerId = PlayerIds.permittedIdForIndex(permittedIndex++);
    ...
    if (permittedPlayers.isEmpty()) {
      plugin.getLogger().fine("No permitted players configured.");
    } else {
