 ...
 *
 * <p>This listener acts as a bridge between the PGM plugin's event system and the Parquet-based
 * match logger. It:
 *
 * <ul>
 *   <li>Filters for Capture the Wool (CTW) matches only
 *   <li>Extracts relevant data from PGM events
 *   <li>Forwards Player objects (not names) to maintain privacy controls
 *   <li>Uses MONITOR priority to observe events without interfering
 * </ul>
 *
 * <p><b>Privacy Note:</b> Player objects are passed to the tracker rather than names, allowing the
 * PositionTracker to apply its anonymization logic based on the permitted players list.
 */
public class PGMEventListener implements Listener {
   ...
   *
   * <p>This handler checks if the match has a WoolMatchModule (indicating CTW mode). Non-CTW
   * matches are ignored. When a CTW match starts, it creates a new Parquet file and begins
   * tracking.
   *
   ...
   *
   * <p>This stops position sampling and writes the match end event to the Parquet file before
   * closing it.
   *
   ...
   *
   * <p>This event fires when a player spawns or respawns during the match. The spawn location is
   * extracted from the event and logged.
   *
   ...
   *
   * <p>The victim's location at the time of death is recorded. This can be used to analyze
   * dangerous areas, engagement zones, or player behavior patterns.
   *
   ...
   *
   * <p>This event is filtered to only log the first touch of a wool objective during a player's
   * current life (using {@code isFirstForPlayerLife()}). This prevents duplicate logs if a player
   * drops and re-picks the same wool.
   *
   ...
   *
   * <p>This event fires when a wool block is successfully placed at a monument, completing the
   * objective. The location logged is where the wool block was placed, not the player's position.
   *
