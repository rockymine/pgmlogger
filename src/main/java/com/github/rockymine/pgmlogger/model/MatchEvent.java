 ...
 *
 * <p>This class uses a sparse schema where not all fields are populated for every event type. The
 * {@code eventType} determines which fields contain data:
 *
 * <ul>
 *   <li>{@code MATCH_START/MATCH_END}: Only timestamp and event type
 *   <li>{@code SPAWN/DEATH}: timestamp, event type, player ID, and position (x,y,z)
 *   <li>{@code POSITION}: All fields except woolId
 *   <li>{@code WOOL_TOUCH/WOOL_CAPTURE}: timestamp, event type, player ID, position, and woolId
 * </ul>
 *
...
public class MatchEvent {
   ...
   *
   * <p>This constructor should generally not be called directly. Use the static factory methods
   * instead ({@link #spawn}, {@link #death}, etc.) which ensure the correct fields are populated
   * for each event type.
   *
  ...
  public static class Serializer implements Dehydrator<MatchEvent> {
     ...
     *
     * <p>Required fields (timestamp, event_type) are always written. Optional fields are only
     * written if non-null.
     *
